...
import 'package:impaktfull_cli/src/cli/util/extensions/list_extensions.dart';
...
class AppCenterPlugin extends ImpaktfullPlugin {
  ...
  Future<void> uploadToAppCenter({
    ...
    CliLogger.verbose('Detected: $appType meme type fo ${file.path}');
    ...
    CliLogger.logSeperator();
    CliLogger.log('Release $releaseId is ready!!! 🎉🎉🎉');
    CliLogger.log(distributionUrl);
    CliLogger.logSeperator();
    await cleanup();
  }
  ...
  Future<Map<String, dynamic>> _createNewRelease({
    ...
    if (response.statusCode != HttpStatus.created) {
      CliLogger.verbose(response.body);
  ...
  Future<void> _splitFile({
    ...
    CliLogger.debug('Split the file into $chunksIndex chunks');
  }
  ...
  Future<void> _uploadChunks({
    ...
    for (final file in tempFiles) {
      ...
      CliLogger.log('Uploading chunks: $chunkProgress');
  ...
  Future<int> _validateRelease({
    ...
    while (releaseId == null && counter < maxPollAttempts) {
      CliLogger.log('Checking if release is ready...');
